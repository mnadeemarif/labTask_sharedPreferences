...
class MainActivity : AppCompatActivity() {
    ...
    fun loginf(view : View){
        ...
        sharedPreferences = getSharedPreferences(myPref, Context.MODE_PRIVATE)
        if (username.getText().toString().equals("") || pass.getText().toString().equals("")){
            Toast.makeText(this,"One of Either fields is empty!",Toast.LENGTH_LONG)
        }
        else{
            if (sharedPreferences.contains(username.getText().toString())){
                var x = sharedPreferences.getString(username.getText().toString(),"-1")
                ...
                Toast.makeText(this,"User not found ! please regiser first !",Toast.LENGTH_LONG)
            }
        }
    */}
}